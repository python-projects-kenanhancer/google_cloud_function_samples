import functions_framework
from flask import Request, Response, make_response

from infrastructure import Settings, inject_settings_from_gcp_storage_env


@functions_framework.http
@inject_settings_from_gcp_storage_env(bucket_name="app-config-boilerplate", blob_name=".env", project_id="nexum-dev-364711")
def settings_from_gcp_storage_env_http(request: Request, settings: Settings) -> Response:

    if settings.feature_flags.circuit_breaker_enabled:
        response = make_response(("Circuit Breaker is enabled", 200))
    else:
        response = make_response((f"Environment variable sql_alchemy_conn={settings.backend_db.sql_alchemy_conn}", 200))

    return response
