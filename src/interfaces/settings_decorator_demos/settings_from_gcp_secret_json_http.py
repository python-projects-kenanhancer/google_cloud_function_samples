...
from infrastructure import Settings, inject_settings_from_gcp_secret_json


@functions_framework.http
@inject_settings_from_gcp_secret_json(secret_name="app-config-json", project_id="nexum-dev-364711")
def settings_from_gcp_secret_json_http(request: Request, settings: Settings) -> Response:

    if settings.feature_flags.circuit_breaker_enabled:
        response = make_response(("Circuit Breaker is enabled", 200))
    else:
